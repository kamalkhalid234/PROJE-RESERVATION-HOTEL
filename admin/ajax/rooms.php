...
if (isset($_POST['add_room'])) {
    ...
    // Correction du type de données pour la requête d'insertion
    if (insert($q1, $values, 'siiiiis')) {
        $flag = 1;
    }

    $rooms_id = mysqli_insert_id($con);
    $q2 = "INSERT INTO `rooms_facilites`(`room_id`, `facilities_id`) VALUES (?,?)";
...
if (isset($_POST['get_room'])) {
    ...
    $res3 = select("SELECT * FROM `rooms_facilites` WHERE `room_id`=?", [$frm_data['get_room']], 'i');
    ...
    if (mysqli_num_rows($res3) > 0) {
        while ($row = mysqli_fetch_assoc($res3)) {
            array_push($facilites, $row['facilities_id']);
        }
    }


    $data = ["roomdata" => $roomdata, "features" => $features, "facilities" => $facilites];
...
if (isset($_POST['edit_room'])) {
    ...
    // تحديث بيانات الغرفة
    $q1 = "UPDATE `rooms`
          SET `name`=?, `area`=?, `price`=?, `quantity`=?, `adult`=?, `children`=?, `description`=?
          WHERE `id` =?";
    ...
    if (update($q1, $values, 'iiiiisi')) {
        $flag = 1;
        error_log("Room updated successfully");
    } else {
        error_log("Failed to update room");
    }

    // حذف الميزات والمرافق المرتبطة
    $del_features = delete("DELETE FROM `room_features` WHERE `room_id` =?", [$frm_data['room_id']], 'i');
    $del_facilities = delete("DELETE FROM `rooms_facilites` WHERE `room_id` =?", [$frm_data['room_id']], 'i');

    if ($del_features && $del_facilities) {
        error_log("Features and facilities deleted successfully");
    } else {
        $flag = 0;
        error_log("Failed to delete features or facilities");
    }

    // إدخال المرافق الجديدة
    $q2 = "INSERT INTO `rooms_facilites`(`room_id`, `facilities_id`) VALUES (?,?)";
    if ($stmt = mysqli_prepare($con, $q2)) {
        foreach ($facilities as $f) {
        ...
        mysqli_stmt_close($stmt);
        error_log("Facilities inserted successfully");
    } else {
        $flag = 0;
        error_log("Failed to prepare facilities insert query");
    }

    // إدخال الميزات الجديدة
    $q3 = "INSERT INTO `room_features`(`room_id`, `features_id`) VALUES (?,?)";
    if ($stmt = mysqli_prepare($con, $q3)) {
        foreach ($features as $f) {
            mysqli_stmt_bind_param($stmt, 'ii', $frm_data['room_id'], $f);
        ...
        mysqli_stmt_close($stmt);
        error_log("Features inserted successfully");
    } else {
        $flag = 0;
        error_log("Failed to prepare features insert query");
    }
...
if (isset($_POST['add_image'])) {
    ...
    if ($img_r == 'inv_img') {
        ...
        $vlaues = [$frm_data['room_id'], $img_r];
        $res = insert($q, $vlaues, 'is');
        echo $res;
    }
}
...
if (isset($_POST['remove_room'])) {
    ...
    $res4 = delete("DELETE  FROM `rooms_facilites` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
